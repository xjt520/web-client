 ...
 */
export type Prettify<T> = T extends DoNotPrettify ? T : {
    [K in keyof T]: T[K];
} & {};
/**
 * Helper function to sets a field in an object
 */
export type SetField<T, F extends string, V> = Prettify<Omit<T, F> & {
    [K in F]: V;
}>;
/**
...
type CamelCaseImpl<S extends string> = S extends `${infer Head}_${infer Tail}` ? `${Head}${Capitalize<CamelCaseImpl<Tail>>}` : S extends `${infer Head}-${infer Tail}` ? `${Head}${Capitalize<CamelCaseImpl<Tail>>}` : S;
/**
 ...
 */
export type SnakeCase<S extends string> = S extends `${infer Head}${infer Tail}` ? Tail extends Uncapitalize<Tail> ? `${Lowercase<Head>}${SnakeCase<Tail>}` : `${Lowercase<Head>}_${SnakeCase<Tail>}` : Lowercase<S>;
type PascalCaseImpl<S extends string> = S extends `${infer Head}_${infer Tail}` ? `${Capitalize<Head>}${PascalCaseImpl<Tail>}` : S extends `${infer Head}-${infer Tail}` ? `${Capitalize<Head>}${PascalCaseImpl<Tail>}` : Capitalize<S>;
/**
 * Convert "some_identifier-name" -> "SomeIdentifierName"
 * - No spaces; allowed separators: "_" and "-"
 ...
 */
export type HasDefaultValue<M> = M extends {
    defaultValue: any;
} ? true : false;
/**
 ...
 */
export type ValidateColumnMetadata<M> = HasDefaultValue<M> extends true ? HasDefaultIncompatibleFields<M> extends true ? InvalidColumnMetadata<'default() cannot be combined with primaryKey(), unique(), or autoInc()'> : M : M;
/**
